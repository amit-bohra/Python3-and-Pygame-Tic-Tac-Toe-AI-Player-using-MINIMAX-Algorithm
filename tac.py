        ...
        while len(stack)!=0:
            ...
            if len(traphlist[strong])==0:
                for i in graphlist[strong]:
                    if i in graphvalue.keys():
                        if level%2==0:
                            tempo=max(graphvalue[i])
                            graph[strong][i]=tempo
                        if level%2==1:
                            tempo=min(graphvalue[i])
                            graph[strong][i]=tempo  #check for level 8
    ...
    if clickhua==True:
        ...
        if string in graph.keys():
            ...
            if 1 in graph[string].values():
                for key,val in graph[string].items():
                    box=int(key[12])
